...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-full overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-200 dark:bg-white",
        className
      )}
      {...props}
    />
...
export function CardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn(
        "flex flex-col space-y-1.5 border-t border-zinc-200 p-4 dark:border-zinc-200",
        className
      )}
      {...props}
    />
...
export function CardTitle({ className, ...props }: CardTitleProps) {
  return (
    <h3
      className={cn(
        "text-lg font-semibold leading-none tracking-tight text-black dark:text-black",
        className
      )}
      {...props}
    />
      ...
      <svg width="356" height="180" xmlns="http://www.w3.org/2000/svg">
        <defs>
          {rectsData.map((rect, index) => {
            ...
            const fillColor = hovered ? rect.hoverFill : rect.fill;
            return (
              <linearGradient
                key={gradientId}
                id={gradientId}
              ...
              >
                ...
                <stop offset="0.05" stopColor={fillColor} stopOpacity="1" />
                <stop offset="0.95" stopColor={fillColor} stopOpacity="0.3" />
        ...
        {rectsData.map((rect, index) => {
          const rectHeight = hovered ? rect.hoverHeight : rect.height;
          const rectY = hovered ? rect.hoverY : rect.y;
          const gradientId = `barGradient-${index}`;
          return (
            <rect
              key={index}
